...
import { Check, ChevronRight, Loader2, ChevronDown } from "lucide-react";
...
const ONLINE_SUPPORT = [
  ...
  "Partially (basic tools only)",
  "No",
];
const INFRASTRUCTURE = [
  "Reliable campus internet",
  "Computer labs",
...
const TIMELINES = [
  "September 2026 (Next academic year)",
  "Early 2027",
  "Later",
];
  ...
  if (isSuccess) {
    return (
      <div className="bg-slate-50 p-8 rounded-2xl text-center border border-slate-200 mt-12 shadow-sm">
        ...
        <h3 className="text-2xl font-bold mb-4 text-slate-900 font-display tracking-tight">
      ...
      <form onSubmit={(e) => { e.preventDefault(); void handleSubmit(onSubmit)(e); }} className="space-y-12">
        <FormSection title="Institution Profile" number="01">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <TextInput
              ...
              placeholder="e.g. Synergia University"
        ...
        <FormSection title="Current Academic Structure" number="02">
          <div className="space-y-6">
            <CheckboxGroup
              label="Programs Currently Offered"
              name="programsOffered"
              options={PROGRAMS_OFFERED}
              control={control}
            />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <SelectInput
                ...
                options={LANGUAGES}
                {...register("instructionLanguage", { required: true })}
                error={errors.instructionLanguage}
              />
              <SelectInput
                label="Current Tuition Range"
                options={TUITION_RANGES}
                {...register("currentTuitionRange", { required: true })}
                error={errors.currentTuitionRange}
              />
            </div>
          </div>
        </FormSection>
        ...
        <FormSection title="Strategic Interest" number="03">
          <div className="space-y-6">
            <CheckboxGroup
              label="Main objectives for international partnerships?"
              name="objectives"
              options={OBJECTIVES}
              control={control}
            />
            <CheckboxGroup
              label="Which types of programs are you most interested in?"
        ...
        <FormSection title="Revenue Position" number="04">
          <div className="space-y-6">
            <RadioGroup
              label="What is the maximum tuition level your institution can realistically support for premium programs?"
              name="maxTuitionPotential"
              options={MAX_TUITIONS}
              control={control}
            />
            <RadioGroup
              label="Are you willing to introduce new premium international programs priced above your current tuition levels?"
              name="willingnessPremium"
              options={YES_POSSIBLY_NO}
              control={control}
            />
          </div>
        </FormSection>
        ...
        <FormSection title="Digital Capacity" number="05">
          <div className="space-y-6">
            <RadioGroup
              label="Does your institution currently support online or distance learning?"
              name="onlineSupport"
              options={ONLINE_SUPPORT}
              control={control}
            />
            <TextInput
              label="What platforms/tools do you currently use?"
              placeholder="e.g. Zoom, Moodle, Google Classroom..."
                ...
                render={({ field }) => (
                  <div className="flex flex-col space-y-4">
                    <input
                      ...
                      max="5"
                      step="1"
                      className="w-full accent-[#0d72ff]"
                      {...field}
                    />
        ...
        <FormSection title="Timeline" number="06">
          <div className="space-y-6">
            <RadioGroup
              label="When would your institution be ready to launch an international program?"
              name="launchTimeline"
              options={TIMELINES}
              control={control}
            />
            <RadioGroup
              label="Estimated number of students for pilot intake"
              name="pilotIntake"
              options={PILOT_INTAKES}
              control={control}
            />
          </div>
        </FormSection>
        ...
        <FormSection title="Internal Readiness" number="07">
          <div className="space-y-6">
            <RadioGroup
              label="Would your institution be open to participating in a pilot international program?"
              name="pilotOpenness"
              options={PILOT_OPENNESS}
              control={control}
            />
            <CheckboxGroup
              label="What internal support can your institution provide?"
              name="internalSupport"
              options={INTERNAL_SUPPORT}
              control={control}
            />
          </div>
        </FormSection>
        ...
        <div className="pt-6 border-t border-slate-100 flex flex-col items-start gap-4">
          {submitError && (
            <div className="bg-red-50 text-red-600 text-sm p-4 rounded-lg border border-red-100 w-full mb-4">
              {submitError}
            </div>
          )}
  ...
  return (
    <div className="relative border-t border-slate-100 pt-6 mt-6 first:border-0 first:pt-0 first:mt-0">
      <div className="flex items-center gap-3 mb-6">
        <span className="text-xs font-bold text-[#0d72ff] bg-blue-50 px-2 py-0.5 rounded uppercase tracking-wider">
          {number}
        </span>
  ...
  ({ label, error, required = true, ...props }, ref) => (
    <div>
      ...
      <input
        ...
        required={required}
        className={cn(
          "w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 text-sm text-slate-900 placeholder:text-slate-400 focus:outline-none focus:bg-white focus:ring-1 focus:ring-[#0d72ff] focus:border-[#0d72ff] transition-all",
...
const SelectInput = React.forwardRef<HTMLSelectElement, any>(
    ...
    <div>
      ...
      <div className="relative">
        <select
          ...
          required={required}
          className={cn(
            "w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-2.5 pr-10 text-sm text-slate-900 focus:bg-white focus:outline-none focus:ring-1 focus:ring-[#0d72ff] focus:border-[#0d72ff] transition-all appearance-none",
            error && "border-red-500 focus:ring-red-500 focus:border-red-500",
          )}
          {...props}
        >
          ...
          render={({ field }) => (
            <>
              {options.map((opt) => (
                <label
                  key={opt}
                  className="relative flex cursor-pointer rounded-lg border border-slate-200 bg-white hover:bg-slate-50 p-3 transition-all hover:border-[#0d72ff]/30 has-[:checked]:border-[#0d72ff] has-[:checked]:bg-blue-50/50"
                >
                  <div className="flex items-start">
                    ...
                    <span className="ml-2.5 text-[13px] font-medium text-slate-700 leading-tight">
                      {opt}
                    </span>
                  </div>
                </label>
            ...
            <>
              {options.map((opt) => (
                <label
                  key={opt}
                  className={cn(
                    "relative flex cursor-pointer rounded-lg border border-slate-200 bg-white hover:bg-slate-50 p-3 transition-all hover:border-[#0d72ff]/30 has-[:checked]:border-[#0d72ff] has-[:checked]:bg-blue-50/50",
                ...
                >
                  <div className="flex items-center">
                    ...
                    <span className="ml-2.5 text-[13px] font-medium text-slate-700">
                      {opt}
                    </span>
                  </div>
                </label>
